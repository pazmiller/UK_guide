...
import { getContributionCities } from './contributionCities';
...
export async function getContributionEntries(): Promise<ExistingEntry[]> {
  ...
  paths.push( 'london/restaurants.ts', 'london/cafes.ts', 'london/attractions.ts' );
  const files: Files = {
  ...
  const cities = getContributionCities();
