...
async function latestReport( issueNumber: number ): Promise<EvaluationReport | null>
{
  ...
  for ( let page = 1; page <= 20; page++ ) {
    ...
    for ( const comment of comments ) {
      if ( comment.performed_via_github_app?.id !== appId || !comment.body.includes( REPORT_PREFIX ) ) continue;
      // A newer malformed assessment invalidates an older one rather than falling back.
      latest = parseEvaluationComment( comment.body );
    }
    if ( comments.length < 100 ) return latest?.issueNumber === issueNumber ? latest : null;
  }
...
{
  const report = await latestReport( issueNumber );
  if ( !report ) {
    ...
    return { report: null, eligible: false, message: '缺少可信的完整评估记录，请重新评估；不能仅凭历史平均分放行。', prUrl: prs.length === 1 ? `https://github.com/${repo}/pull/${prs[0].number}` : null };
  }
  const repo = publicRepository();
  ...
  return { report, prUrl, eligible: pr.draft && judgeOnlyFailure( report ), message: !pr.draft ? 'PR 已为 Ready；原 AI 评分仍保留。' : judgeOnlyFailure( report ) ? '仅 Judge 分数未达标，其他检查已通过。人工放行不会合并 PR。' : '存在其他检查失败，不允许人工绕过。' };
}
...
export async function manuallyApproveContribution( issueNumber: number, headSha: string, reason: string, actor: string )
{
  ...
  if ( !matchingReviewCommit( report, after, repo ) ) {
  ...
  if ( !matchingReviewCommit( report, finalPr, repo ) ) {
