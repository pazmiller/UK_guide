...
- 很多同学在签证中心或抵达前后会拿到赠送 SIM 卡，这类卡可以临时过渡，但不建议长期绑定银行、学校账户、邮箱二次验证。尽快考虑更换正式 SIM。
- 办卡前先用学校宿舍、租房 postcode 查询信号覆盖；可用 [Signalchecker](https://www.signalchecker.co.uk/) 查询信号覆盖，再用 [uSwitch](https://www.uswitch.com/mobiles/compare/sim_only_deals/) 方便对比套餐价格，然后选择运营商。
**重要：** - 遇到不确定的短信不知道是不是诈骗，问群友+AI先

| 使用情境 | 建议 |
| --- | --- |
| 伦敦、曼城、伯明翰、利兹等大城市 | 比较价格、流量和 eSIM 支持情况。可查看 [EE](https://ee.co.uk/)、[O2](https://www.o2.co.uk/)、[Vodafone](https://www.vodafone.co.uk/)、[Three](https://www.three.co.uk/)、[giffgaff](https://www.giffgaff.com/)，以及 VOXI、SMARTY、Lebara 等虚拟运营商。 |
| 小地方，或乡村校区 | 不要只看价格，先用宿舍、教学楼和常去区域的 postcode 查信号覆盖。 |
| 经常给国内打电话 | 可比较 Lebara、Lyca 等是否包含国际通话分钟数，并确认中国大陆号码是否在套餐范围内。 |
...
- 不要在马路边、地铁口、公交站或酒吧门口长时间举着手机。
- 步行导航尽量靠内侧，手机拿在远离马路的一侧。
- 骑车或电动车抢手机在伦敦、曼城等大城市中心区域较常见。
- 不要边走边戴降噪耳机，完全隔绝环境声音。
- 夜间尽量走主路、明亮且人多的地方。
- 不要把手机、护照和银行卡放在同一个包里。
- 开启 Find My iPhone／Find My Device，并记录 IMEI 号码。
- 设置 SIM PIN、强密码、Face ID／Touch ID；银行 app、邮箱和学校账号都开启双重验证。

| 电话 | 使用情况 |
| --- | --- |
| `999` | 正在发生的危险、有人受伤或感觉危险。 |
| `101` | 非紧急报警或补充报案信息。 |
| `111` | 非紧急医疗建议，按当地规则使用。 |
...
1. 先保证人身安全，不要追抢匪。
2. 正在发生、有人受伤或感觉危险时，打 `999`。
3. 非紧急情况打 `101` 或在线报警。
4. 远程锁定或擦除手机。
5. 联系运营商停用 SIM。
6. 联系银行冻结卡和 app。
7. 保留 crime reference number，用于保险理赔。
8. 修改邮箱、银行和学校账号密码。

## 5. 签证与身份状态：确认 eVisa

- 英国已逐步使用 eVisa 作为数字身份状态；具体状态以 [GOV.UK eVisa](https://www.gov.uk/evisa) 与 UKVI account 为准。
- 到英国前后检查 UKVI account 能否正常登录。
- 确认护照信息和 eVisa 绑定正确；换护照、改姓名或发现签证信息有误时及时更新。
- 保存 share code 使用方式；重要场景登录官方系统生成最新状态，不要只依赖截图。
- 保存学校 visa compliance／international student support 的联系方式。

参阅：[View and prove your immigration status](https://www.gov.uk/view-prove-immigration-status)。

## 6. 医疗：尽快注册 GP

NHS 的 GP（General Practitioner）是 primary care doctor，即全科医生，接近国内普通内科，是英国医疗系统的第一入口。通常需经 GP referral 才能看内分泌科、心脏科等 specialist；GP 主要进行基础问诊、普通检查及转诊判断。

- 到英国后尽快按住址注册 GP；不同地区和诊所流程可能不同。
- 对已支付 IHS 或符合资格的留学生，大多数 NHS 服务不像私立医疗按次收费；英格兰处方药通常按每个 item 收固定费用，页面信息为 £9.90。
- 专科或转诊等待时间可能从数周到数月，1–3 个月并不罕见；尽快注册 GP，有病尽早联系 GP 或 NHS 111。
- 紧急生命危险打 `999`；非紧急医疗建议可用 NHS 111。
- “立刻就诊”基本仅见于 A&E 急诊；非严重或非急性疾病到 A&E 也可能等待数小时。
- 牙医通常需单独注册，NHS dentist 名额可能紧张。
- 常用非处方药可在 Boots、Superdrug、药房或超市购买。
- 带药入境前确认是否需要英文处方或医生证明。
- 眼镜、牙科和处方药可能有额外费用，提前了解。
- 可向群友了解本地 GP、药房、牙医和 A&E 的经验。
...
- 没看房、没合同、没确认 landlord／agent 身份前，不要转大额押金。
- 确认 deposit 是否进入 tenancy deposit protection scheme（第三方押金保护平台，防止房东无故扣押金）。
- 确认 bills 是否 included：水、电、气、网、council tax。
- 学生通常可能有 council tax exemption，但须按当地 council 要求提交证明。
- 入住当天拍照记录房间状态，尤其是墙面、地毯、家具和电器。
- 确认 smoke alarm、carbon monoxide alarm 和门锁是否正常。
- 不要随便把宿舍门禁卡借给陌生人。
- 学校的学生公寓上述问题基本可以免除，只剩下 ensuite 的祈祷能碰上干净一点、负责一点的室友。

## 9. 小心诈骗：They are everywhere

- 接到自称银行、警察、DPD、Amazon、Royal Mail、HMRC、NHS、快递或大使馆的电话，不要直接转账、报验证码或交出账户信息。
- 任何要求把钱转到“安全账户”、任何涉及到钱的说法，除了是群主叫你捐钱外，都先停下来核实。
- 短信链接不确定就不要点；自己打开官网或 app 核对。
- 不要以为骗局只会找上“容易受骗的人”；每年都有留学生受骗。
- 觉得不对劲时，先问群内、银行官方客服或学校；已经被骗时尽快联系银行冻结交易，并按情况拨打 999 报警。

有朋友的警察诈骗案拟真到电话号码模拟，知道你的身份证等家人信息，还会假装像是从真的警察局打给你一样。

案例阅读：[知乎案例](https://zhuanlan.zhihu.com/p/1986517925008527666)、[留学生被骗案例](https://www.51offer.com/article/detail_108905.html)、[洗钱风险案例](https://jurify.co.uk/blog/uk-chinese-students-money-laundering-case)。

## 10. 学术规则：不要踩 plagiarism 等红线

- 认真阅读学校的 academic integrity policy。
- 不同课程对 ChatGPT／AI tools 的使用规则不同。
- 引用、改写、代码协作和代写都要严格遵守学校规定。
- 英国学校可能因 academic misconduct 劝退学生；本科阶段通常更严格。
- 小组作业保留自己的贡献记录。
- 不确定时问 tutor，不要听信社交平台或群聊的模糊说法。

另请查看：[GOV.UK Student visa](https://www.gov.uk/student-visa)。

## 11. 其他容易忽略的事项

- 插头：英国使用 Type G 三孔插头，提前准备转换头。
- 天气：冬天阴冷潮湿，注意防水外套、保暖和除湿。
- 保险：考虑手机险、财产险和旅行险。
- 快递：[Royal Mail](https://www.royalmail.com/)、[Evri](https://www.evri.com/)、[DPD](https://www.dpd.co.uk/) 与 [Amazon Locker](https://www.amazon.co.uk/ulp) 的取件方式不同。
- 垃圾分类：不同 council 的规则不同，按住址查询 bin collection day。
- 紧急联系人：保存学校 security、宿舍 reception、朋友、GP 和保险电话。
- 兼职：确认 [Student visa 的工作时长限制](https://www.gov.uk/student-visa)，不要做 cash-in-hand 或可疑工作。
- 交通安全：英国靠左行驶；过马路先看右，再看左。
- Pub：酒吧是英国本地人不得不尝的周常甚至是日常；每周五大家一起去聚一聚来一 pint，玩玩 pub game 或者干聊天也是蛮不错的。
- 不要接别人递来的奇怪香烟；不确定时不要尝试，避免“飞叶子”或其他不了解的物质。
...
- [Police 999 / 101](https://www.police.uk/pu/contact-the-police/report-a-crime-incident/)
- [GOV.UK eVisa](https://www.gov.uk/evisa)
