...
test( 'wire records fail closed and both repositories share the same contract', async () => {
  ...
  if ( existsSync( '.agent-automation/src/evaluation-report.ts' ) ) assert.equal( await readFile( 'lib/contributions/evaluation.ts', 'utf8' ), await readFile( '.agent-automation/src/evaluation-report.ts', 'utf8' ) );
} );
